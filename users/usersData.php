...
<head>
    ...
    <title>Users | Hypermartindia</title>
                ...
                <table id="example" class="table table-bordered table-responsive-sm" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th>Mobile No</th>
                            <th>Role</th>
                            <?php if($_SESSION['userRole'] == 'admin' || $_SESSION['userRole'] == 'edit') { ?>
                            <th>Action</th>
                            <?php } ?>
                        </tr>
                    </thead>
                        ...
                        <?php
                            ...
                            foreach ($users as $user) { 
                                if($user['userName'] !== 'admin') { ?>
                                    <tr id="<?php echo $user['_id'] ?>">
                                    <td> <?php echo $user['userName'] ?> </td>
                                    <td> <?php echo $user['firstName']." ".$user['lastName']?></td>
                                    <td> <?php echo $user['email'] ?> </td>
                                    <td> <?php echo $user['mobileNo'] ?> </td>
                                    <td> <?php echo $user['userRole'] ?> </td>
                                    <?php if($_SESSION['userRole'] == 'edit' ) { ?>
                                        <td><a href="updateUser.php?id=<?php echo $user['_id']; ?>" class="btn btn-primary editUser" id="edit<?php echo $user['_id'] ?>">Edit</a></td>  
                                    <?php } ?>
                                    <?php if($_SESSION['userRole'] == 'admin') { ?>
                                        <td><a href="updateUser.php?id=<?php echo $user['_id']; ?>" class="btn btn-primary editUser" id="edit<?php echo $user['_id'] ?>">Edit</a>
                                            <button href="delete.php" class="btn btn-danger delete" id="<?php echo $user['_id'] ?>">Delete</button></td>
                                    <?php } ?>
                                </tr>
                            <?php } 
                            }
